...
API_DEV_KEY = '68VHhAkQMNBzEfqWlaK3shfj8OdCRM8u'
...
def post_new_paste(title, body_text, expiration='N', listed=True):
    ...
    #else:
    #    api_paste_name = 1
    #This is the same as the 0 if listed else 1
    ...
    #Send the POST request and get the respond

    resp_msg = requests.post(PASTEBIN_API_POST_URL, data=params)

    #check whether the request was successful
    if resp_msg.status_code == requests.codes.ok:
        print(f'New Paste created successful: {resp_msg.text}')
        return resp_msg.text
    else:
        print(f'Request failed: {resp_msg.text}')
        print(f'States Code: {resp_msg.status_code}')
        print(f'Reason: {resp_msg.reason}')

    return

def main():
    post_new_paste('Testing', 'Hello, my name is Justin the flaming bird \nThis will delete soon', '1H', False)
