...
def process_order_data(data_csv, workshops_path):
    ...
    df.sort_values('ITEM NUMBER', ascending=True, inplace=True)
...
def main():
    ...
    #REQ-3 If path was found, check if CSV is called for, if not error message and exit
        ...
        if not data_csv.endswith('.csv'):
            ...
            #Create name for new folder per time use
            #normal_path = '/Orders_' + str(today_date) 
            #data_path = order_path + normal_path
